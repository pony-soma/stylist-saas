...
function fixture(options = {}) {
  ...
  const compiled = ts.transpileModule(fs.readFileSync('app/api/notify/booking/route.ts', 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText;
  const exports = {};
  vm.runInNewContext(compiled, { exports, require: id => mocks[id], process: { env: { LINE_LOGIN_CHANNEL_ID: 'channel', LINE_CHANNEL_ACCESS_TOKEN: 'dummy', LINE_BOOKING_NOTIFICATIONS_ENABLED: options.disabled ? 'false' : 'true' } }, Date, AbortSignal, console: { error() {} }, fetch: async (url, init) => { requests.push({ url, init }); return url.includes('/verify?') ? Response.json({ client_id: options.wrongChannel ? 'other' : 'channel', expires_in: options.expired ? 0 : 100 }, { status: options.badToken ? 401 : 200 }) : Response.json({ userId: customerLine }); } });
  ...
  return { post: exports.POST, request, sent, requests, reads };
}
