...
const catalog=schemas=>`SELECT json_build_object(
 ...
 'functions',(SELECT json_agg(t ORDER BY schema,name,args) FROM (SELECT n.nspname AS schema,p.proname AS name,pg_get_function_identity_arguments(p.oid) AS args,pg_get_functiondef(p.oid) AS definition,(SELECT array_agg(acl::text ORDER BY acl::text) FROM unnest(p.proacl) acl) AS proacl,pg_get_userbyid(p.proowner) AS owner FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname IN (${schemas}) AND p.prokind IN ('f','p')) t)
)::text;`;
