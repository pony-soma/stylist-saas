...
export async function notifyBookingRequest(req: Request) {
  ...
  // Only enable after the verified booking flow and the official account are configured.
  ...
  try {
    ...
    if (!response.ok && !(response.status === 409 && response.headers.get('x-line-accepted-request-id'))) {
      console.error('LINE booking push rejected', { status: response.status });
