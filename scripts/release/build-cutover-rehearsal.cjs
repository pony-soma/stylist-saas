...
const rewrite=s=>s.replaceAll('public.','lino_cutover_rehearsal.').replaceAll("schemaname='public'","schemaname='lino_cutover_rehearsal'").replaceAll("schemaname = 'public'","schemaname = 'lino_cutover_rehearsal'");
const sql=`begin;
...
reset role;
rollback;
select 'PASS: atomic cutover rehearsal; one master; tester not exempt; legacy rows preserved and hidden; all synthetic changes rolled back' as result;
`;
